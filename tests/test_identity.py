...
LEARN_FUNC_DICT = {
    'a2c': lambda e: A2C(policy=native_str("MlpPolicy"), learning_rate=1e-3, n_steps=1,
                         gamma=0.7, env=e).learn(total_timesteps=10000, seed=0),
    'acer': lambda e: ACER(policy=native_str("MlpPolicy"), env=e,
                           n_steps=1, replay_ratio=1).learn(total_timesteps=15000, seed=0),
    'acktr': lambda e: ACKTR(policy=native_str("MlpPolicy"), env=e,
                             learning_rate=5e-4, n_steps=1).learn(total_timesteps=20000, seed=0),
    'dqn': lambda e: DQN(policy=native_str("MlpPolicy"), batch_size=16, gamma=0.1,
                         exploration_fraction=0.001, env=e).learn(total_timesteps=40000, seed=0),
    'ppo1': lambda e: PPO1(policy=native_str("MlpPolicy"), env=e, lam=0.5,
                           optim_batchsize=16, optim_stepsize=1e-3).learn(total_timesteps=15000, seed=0),
    'ppo2': lambda e: PPO2(policy=native_str("MlpPolicy"), env=e,
                           learning_rate=1.5e-3, lam=0.8).learn(total_timesteps=20000, seed=0),
    'trpo': lambda e: TRPO(policy=native_str("MlpPolicy"), env=e,
...
def test_identity_ddpg():
    ...
    model = DDPG(native_str("MlpPolicy"), env, gamma=0.0, param_noise=param_noise, memory_limit=int(1e6))
